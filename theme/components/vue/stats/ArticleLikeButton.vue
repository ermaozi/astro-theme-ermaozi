...
import { getEngagementStatus, LIKE_UPDATE_EVENT, removeLike, submitLike } from './like-api.js'
...
const formattedCount = computed(() => {
  ...
  return new Intl.NumberFormat(props.lang, { notation: 'compact', maximumFractionDigits: 1 })
...
const scheduleNextDayRefresh = () => {
  ...
  dayRefreshTimer = window.setTimeout(() => loadStatus(true), nextMidnight.getTime() - Date.now() + 1500)
}
...
onMounted(() => {
  mounted = true
  window.addEventListener(LIKE_UPDATE_EVENT, syncLike)
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener(LIKE_UPDATE_EVENT, syncLike)
...
<template>
  ...
  <Teleport v-if="heartBursts.length" to="body">
      <span
