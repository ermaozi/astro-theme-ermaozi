...
test('documentation navigation and content tools work', async ({ page, context }) => {
  ...
  await expect(nestedItems).toHaveClass(/fade-in-height-expand-leave-active/)
  await expect(nestedItems).toBeHidden()
  await nestedItem.focus()
  await page.keyboard.press('Enter')
  await expect(nestedItems).toHaveClass(/fade-in-height-expand-enter-active/)
...
test('external code embeds preserve Plume URLs, options, theme sync, and sandboxing', async ({ page }) => {
  ...
  await expect(fiddle).toHaveAttribute('src', /jsfiddle\.net\/pengzhanbo\/1xbwz2p9\/embedded\/result%2Cjs$/)
