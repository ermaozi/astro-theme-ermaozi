...
test('mobile navigation and local document navigation keep the frozen matrix', async ({ page }) => {
  ...
  for (const theme of ['light', 'dark'] as const) {
    for (const width of [390, 639, 767]) {
      ...
      await page.locator('.vp-nav-screen-translations > .title').click()
      await expect(page.locator('.vp-nav-screen-translations .link')).toHaveCSS('font-size', '13px')
...
test('Plume root, page state, and desktop shell geometry stay aligned', async ({ page }) => {
  ...
  await expect(page.locator('.vp-sidebar .vp-menu-badge')).toHaveCSS('font-size', '10px')
