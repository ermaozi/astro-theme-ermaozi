...
test('build contains generic blog, docs, enhanced Markdown, search, and crawler files', async () => {
  ...
  assert.match(post, /hreflang="en-us"/i)
  ...
  assert.match(configuration, /class="vp-file-tree-info file add diff"/)
  assert.match(configuration, /class="vp-file-tree-info file remove diff"/)
  ...
  assert.match(configuration, /class="vp-file-tree-node generated"/)
...
test('all frozen Plume comment providers remain available as lazy chunks', async () => {
  const [packageJson, files, component, page, schema] = await Promise.all([
    ...
    readdir('dist/_astro'),
    readFile('src/components/vue/Comments.vue', 'utf8'),
...
test('profile-disabled blogs retain Plume local taxonomy navigation', async () => {
  const [page, home, nav, styles] = await Promise.all([
    readFile('src/pages/[...path].astro', 'utf8'),
    readFile('src/components/CustomHome.astro', 'utf8'),
    readFile('src/components/PostsNav.astro', 'utf8'),
    readFile('src/styles/plume-parity.css', 'utf8'),
  ])
...
test('page navigation uses one shared Plume-compatible partial navigation module without an SPA router', async () => {
  const [layout, sidebar, profile, navigation, header, styles] = await Promise.all([
    ...
    readFile('src/components/DocsSidebar.astro', 'utf8'),
    readFile('src/components/ProfileAside.astro', 'utf8'),
    readFile('src/lib/page-navigation.ts', 'utf8'),
    readFile('src/components/Header.astro', 'utf8'),
    readFile('src/styles/plume-parity.css', 'utf8'),
  ])
