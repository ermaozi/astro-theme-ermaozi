export const configureSiteConfig = (source, { siteName, siteDescription, multilingual }) => {
  ...
  let configured = source.replace(
    multilingualPattern,
    (_, prefix) => `${prefix}${multilingual},`,
  )

  let siteNameCount = 0
  configured = configured.replace(/^(\s*siteName:\s*).+,$/gm, (_, prefix) => {
    siteNameCount += 1
  ...
  const described = chinese.replace(descriptionPattern, (_, prefix) => `${prefix}${JSON.stringify(siteDescription)},`)
